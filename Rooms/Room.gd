...
onready var entrance: Node2D = get_node("Enterance")
...
func _close_enterence() -> void:
	for entrey_position in entrance.get_children():
		tile_map.set_cellv(tile_map.world_to_map(entrey_position.global_position), 1)
		tile_map.set_cellv(tile_map.world_to_map(entrey_position.global_position)+ Vector2.DOWN, 2)
